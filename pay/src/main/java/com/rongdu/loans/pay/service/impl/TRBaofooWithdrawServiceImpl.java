 ...
 * @author sunda
 */
@Service("trBaofooWithdrawService")
public class TRBaofooWithdrawServiceImpl extends BaseService implements TRBaofooWithdrawService {
	...
	public void withdraw(RepayLogVO order, String cnName, String idNo, String mobile, String accountid,
		...
		pw.setTerminalId(BaofooConfig.tr_baofoo_withdraw_terminal_id);
	...
	public boolean sendWithdrawCommand(PayLogVO pw) {
		...
		String keyStorePath = this.getClass().getResource(BaofooConfig.tr_baofoo_keystore_path).getPath();
		String keyStorePassword = BaofooConfig.tr_baofoo_keystore_password;
		String pubKeyPath = this.getClass().getResource(BaofooConfig.tr_baofoo_pubkey_path).getPath();
		...
		try {
			...
			// 发送请求
			String requestUrl = BaofooConfig.tr_baofoo_withdraw_url;
			// 商户号
			String memberId = BaofooConfig.tr_baofoo_member_id;
			// 终端号
			String terminalId = BaofooConfig.tr_baofoo_withdraw_terminal_id;
	...
	public TransContent<TransRespBF0040002> queryWithdrawResult(String orderNo) throws Exception {
		...
		// 发送请求
		String requestUrl = BaofooConfig.tr_baofoo_withdraw_result_url;
		// 商户号
		String memberId = BaofooConfig.tr_baofoo_member_id;
		// 终端号
		String terminalId = BaofooConfig.tr_baofoo_withdraw_terminal_id;
	...
	public TaskResult processUnsolvedOrders() {
		...
		List<PayLogVO> pwList = payLogService.findTRBaofooPayUnsolvedOrders(BAOFOO_PAY_UNSOLVED_STATUS);
...
//							}
	...
	public boolean payment(PayLogVO pw) {
		pw.setId("TR" + DateUtils.getHHmmss() + (int) (new Random().nextInt(900) + 100));
		pw.setIsNewRecord(true);
		pw.setTxType("withdraw");
		pw.setTxDate(Integer.parseInt(DateUtils.getDate("yyyyMMdd")));
		pw.setTxTime(new Date());
	...
	public String queryBaofooBalance(String account_type) {
		...
		PostParams.put("member_id", BaofooConfig.tr_baofoo_member_id);// 商户号
		PostParams.put("terminal_id", BaofooConfig.tr_baofoo_withdraw_terminal_id);
		...
		String KeyString = BaofooConfig.tr_baofoo_keystore_password;
