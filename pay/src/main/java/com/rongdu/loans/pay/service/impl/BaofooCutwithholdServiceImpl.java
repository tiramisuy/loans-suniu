...
import com.rongdu.loans.pay.message.CutwithholdDataContent;
import com.rongdu.loans.pay.message.CutwithholdQueryDataContent;
...
@Service(value = "baofooCutwithholdService")
public class BaofooCutwithholdServiceImpl extends PartnerApiService implements BaofooCutwithholdService {

	// 配置参数
	private String partnerId = BaofooConfig.partner_id;
	private String partnerName = BaofooConfig.partner_name;
	private String bizCode = BaofooConfig.cutwithhold_biz_code;
	private String bizName = BaofooConfig.cutwithhold_biz_name;
	// 商户号
	private String memberId = BaofooConfig.cutwithhold_member_id;
	// 分账商户号
	private String cutMemberId = BaofooConfig.cutwithhold_cut_member_id;
	// 终端号
	private String terminalId = BaofooConfig.cutwithhold_terminal_id;
	 ...
	 * 宝付代扣
	 * 
	 * 1.调用宝付代扣接口 2.如果调用代扣接口结果未知，主动查询结果， 3.如果代扣成功，调用代付接口；如果结果未知，返回未知失败状态
	 * 
	 * @param param
	 * @param payType
	 *            code y0524
	...
	private RepayLogVO saveRepayLog(WithholdResultVO vo, WithholdOP param, Integer payType) {
		...
		if (vo.getSuccess()) {
			BigDecimal succAmt = new BigDecimal(vo.getSuccAmt()).divide(BigDecimal.valueOf(100),
	...
	public WithholdResultVO transaction(WithholdOP param) {
		...
		CutwithholdDataContent content = new CutwithholdDataContent();
		...
		// 分账信息
		int cutMerberAmt = 100;
		int merberAmt = Integer.parseInt(param.getTxnAmt()) - cutMerberAmt;
		StringBuffer shareInfo = new StringBuffer();
		shareInfo.append(cutMemberId).append(",").append(cutMerberAmt).append(";");
		shareInfo.append(memberId).append(",").append(merberAmt);
		content.setShare_info(shareInfo.toString());

		// 分账手续费商户
		content.setFee_member_id(memberId);
	...
	public WithholdQueryResultVO query(WithholdQueryOP param) {
		...
		CutwithholdQueryDataContent content = new CutwithholdQueryDataContent();
	...
	private Object sendRequest(BaseDataContent content, Class<?> clazz) {
		// 接口地址
		String url = BaofooConfig.cutwithhold_url;
		// 商户私钥
		String pfxpath = this.getClass().getResource(BaofooConfig.cutwithhold_member_prikey_path).getPath();
		// 宝付公钥
		String cerpath = this.getClass().getResource(BaofooConfig.cutwithhold_baofoo_pubkey_path).getPath();
		// 数字证书密码
		String keyPassword = BaofooConfig.cutwithhold_keystore_password;
		...
		try {
			String reqString = JsonMapper.toJsonString(content);
			logger.debug("{}-{}-请求报文加密数据：{}", partnerName, bizName, reqString);
			String base64str = SecurityUtil.Base64Encode(reqString);
			String dataContent = RsaCodingUtil.encryptByPriPfxFile(base64str, pfxpath, keyPassword);
			...
			logger.debug("{}-{}-应答结果密文：{}", partnerName, bizName, respString);
