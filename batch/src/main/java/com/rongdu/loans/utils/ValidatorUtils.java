...
public class ValidatorUtils {
    ...
    //匹配电话号码
    private static final String phone = "((\\(\\d{2,3}\\))|(\\d{3}\\-))?(\\(0\\d{2,3}\\)|0\\d{2,3}-)?[1-9]\\d{6,7}(\\-\\d{1,4})?";
    //匹配手机号码
    private static final String mobile = "((\\(\\d{2,3}\\))|(\\d{3}\\-))?(1[3458]\\d{9})";
    //匹配网址
    ...
    public static class Result {
    	...
    	public String toString() {
    		return new StringBuilder().append("match:").append(match).append(",msg:").append(msg).toString();
    	}
    }
    ...
    public static boolean required(String value){
    	boolean match = isNotNull(value)&&isNotBlank(value)&&!"null".equalsIgnoreCase(value);
    	return match;
    }
   ...
   public static Result phone(String label,String value){
    ...
    public static Result rangelength(String label,String value,int min,int max){
