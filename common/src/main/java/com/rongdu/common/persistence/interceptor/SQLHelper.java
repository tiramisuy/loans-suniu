...
public class SQLHelper {
	...
	public static int getCount(final String sql, final Connection connection, final MappedStatement mappedStatement,
		...
		if ("oracle".equals(dbName)) {
			...
			countSql = "select count(1) from (" + removeCountField(removeOrders(sql)) + ") tmp_count";
		}
	 ...
	 * @return
	 */
	private static String removeCountField(String qlString) {
		String select = "select";
		String from = "from";
		String selcetSql = qlString.substring(0, qlString.toLowerCase().indexOf(select) + select.length());
		String fromSql = qlString.substring(qlString.toLowerCase().indexOf(from), qlString.length());
		return selcetSql + " 0 " + fromSql;
	}
}
