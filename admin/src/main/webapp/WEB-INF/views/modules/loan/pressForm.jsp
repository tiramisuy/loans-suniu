...
<head>
    ...
    <style type="text/css">
    	...
    	#contactConnectionInfo .layui-elem-field{height:458px!important}
...
<div class="layui-tab layui-tab-card">
    <ul class="layui-tab-title">
        ...
        <c:if test="${overday > 1}">
	       <%-- <li onclick="getReportPage('${userIdNo}','${userRealName}','${userMobile}')">资信云报告</li>--%>
...
<%--		<li onclick="locationInfo('${userId}')">定位信息</li>
		<li onclick="cardinfoView('${userId}')">银行卡信息</li>
		<li onclick="contactApply('${applyId}','${userId}','${userRealName}','${userMobile}')">查重信息</li>--%>
    </ul>
...
<script id="tpl_userinfo" type="text/x-handlebars-template">
    <%--客户信息--%>
    <jsp:include page="../cust/userInfo.jsp"></jsp:include>
</script>
...
<script id="tpl_3" type="text/x-handlebars-template">
    <%--银行卡信息--%>
    <jsp:include page="../cust/cardinfo.jsp"></jsp:include>
</script>
<script id="tpl_6" type="text/x-handlebars-template">
    <%--查重信息--%>
    <jsp:include page="../cust/contactApply.jsp"></jsp:include>
</script>
<script id="tpl_9" type="text/x-handlebars-template">
    <%--定位信息--%>
    <jsp:include page="../cust/locationInfo.jsp"></jsp:include>
</script>
<script id="tpl_11" type="text/x-handlebars-template">
    <%--资信云1--%>
    <jsp:include page="../cust/reportData1.jsp"></jsp:include>
</script>
<script id="tpl_12" type="text/x-handlebars-template">
    <%--资信云2--%>
    <jsp:include page="../cust/reportData2.jsp"></jsp:include>
</script>
...
<script>
    ...
    function contactConectInfo(applyId, userId) {
        ...
        $.ajax({
            ...
            success: function (data, textStatus) {
                if (data.code == "1") {
                    var myTemplate = Handlebars.compile($("#tpl_contactConnectInfo").html());
...
/*         if ( type == '') {
    ...
    function returnParent(){
    	...
    	//页签模式下parentId有值，非页签模式没有值
    	if(parentId != "" && parentId != "undefined"  ){
    		//将父tab页面的搜索条件更新到子页面，之后关闭当前tab页，选中父tab页
    		var info = this.parent.getCollectionPage(parentId);
	   	 	fillForm(info);
    		this.parent.closeTab();
 	    	this.parent.freshTab(parentId);
    	}	
    	//如果是点击上一个，下一个的时候  页码应该不和父tab保持一致，需要和之前计算出来的一致
    	if(beforeOrAfter == '1' || beforeOrAfter == '-1'){
    		$("#pageNo").val(pageNo);
    	}
		var searchFormUrl = $("#checkForm").serialize();
		//避免中文搜索条件get请求乱码问题    页面加上转码，controller进行解码    
		var userNameEncode = "";
