...
<div class="layui-tab layui-tab-card">
    <ul class="layui-tab-title">
        ...
        <c:if test="${sign == 'check'}">
            <li onclick="approveView('${applyId}','${vo.userInfoVO.id}','${vo.userInfoVO.realName}','${vo.userInfoVO.mobile}')">
                审批
            </li>
        </c:if>
        ...
        <c:if test="${sign == 'check' or sign == 'detail' }">
            <li onclick="xianJinCardData('${applyId}')">奇虎360报告</li>
        </c:if>
        <c:if test="${sign == 'check' or sign == 'detail' }">
...
<script id="tpl_7" type="text/x-handlebars-template">
    <%--客户信息--%>
    <jsp:include page="sllUserInfo.jsp"></jsp:include>
</script>
<script id="tpl_8" type="text/x-handlebars-template">
    <%--通讯录信息--%>
    <jsp:include page="sllConnectInfo.jsp"></jsp:include>
</script>
...
<script id="tpl_16" type="text/x-handlebars-template">
    <%--现金白卡报告--%>
    <jsp:include page="sllData.jsp"></jsp:include>
</script>
...
<script>
    ...
    function applyCheck(form, productId) {
        ...
        if (productId == 'TFL' || productId == 'LYFQ') {
        	...
        	if(form.checkResult.value == '1' && form.approveTerm.value == '28' && form.approveAmt.value!='2000'){
        		alert("28天产品审批金额只能为2000");
            	return false;
        	}
        }
        if (productId == 'XXX') {
    ...
    function contactConectInfo(applyId, userId) {
        ...
        $.ajax({
            type: "post",
            url: "${ctx}/sys/custUser/getSLLConnectInfo",
            data: param,
            dataType: "json",
            error: function (request) {
                alert("系统繁忙,请稍后再试");
