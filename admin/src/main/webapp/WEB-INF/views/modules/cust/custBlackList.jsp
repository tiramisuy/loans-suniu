...
<body>
	...
	<ul class="nav nav-tabs">
    	<li class="active">
                <a href="${ctx}/sys/custUser/custBlackList">黑名单</a>
    	</li>
	</ul>
	...
	<form:form id="searchForm" modelAttribute="BlacklistOP" action="${ctx}/sys/custUser/custBlackList" method="post" class="breadcrumb form-search layui-form">
	...
	<table id="contentTable" class="table">
		...
		<tbody>
			<c:forEach items="${page.list}" var="item" varStatus="xh">
				...
				<td>${item.reason }</td>
				<td>
					<a href="javascript:void(0)" data-method="offset" data-type="auto" style="padding-left: 5px;" onclick="deleteBlickList('${item.id}')">移除黑名单</a>
				</td>
				</tr>
			</c:forEach>
		</tbody>
	...
	<script>
				...
				function deleteBlickList(blackId) {
					layer.confirm('您确定要移除黑名单吗？', {
						btn : [ '提交', '取消' ]
					}, function(index) {
						deleteConfirm(blackId);
						layer.close(index);
					});
				}
			
				function deleteConfirm(blackId) {
					var param = {
							blackId : blackId
					};
					$.ajax({
						type : "post",
						url : "${ctx}/sys/custUser/deleteBlickList",
						data : param,
						dataType : "json",
