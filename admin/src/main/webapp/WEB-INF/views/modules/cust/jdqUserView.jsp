...
<div class="layui-tab layui-tab-card" style="position:fixed; top:30px;width:100%;z-index: 99999;margin: 0px;" id="floatDiv">
    <ul class="layui-tab-title">
        <c:if test="${sign == 'check' or sign == 'detail' }">
            <li class="layui-this" onclick="skipHrefLocation('topAPoint');contactApply('${applyId}','${vo.userInfoVO.id}','${vo.userInfoVO.realName}','${vo.userInfoVO.mobile}')">
                查重信息
            </li>
        </c:if>
        ...
        <c:if test="${sign == 'check' or sign == 'detail' }">
            <li onclick="skipHrefLocation('topAPoint');xianJinCardData('${applyId}')">借点钱报告</li>
            <li onclick="skipHrefLocation('topAPoint');anRongData('${applyId}')">安融报告</li>
        </c:if>

        <li>
            <a href="javascript:skipHrefLocation('topAPoint')">${vo.userInfoVO.realName} - ${vo.userInfoVO.mobile} - ${channelMap[applyInfo.channelId]}</a>
        </li>
    ...
    <c:if test="${!empty vo.juqianbao}">
        <table class="layui-table">
            ...
            <c:forEach var="item" items="${vo.juqianbao}" varStatus="xh">
                <tr>
                <td>${item.name}</td>
                <td>${item.ruleCode}   ${item.ruleCode == 854482}  ${item.ruleCode == '854482'}</td>
                <c:choose>
        ...
        <table class="layui-table">
            ...
            <c:forEach var="item" items="${vo.third}" varStatus="xh">
                <c:choose>
                    ...
                    <c:when test="${item.ruleCode == '10190003' or item.ruleCode == '10030057' or item.ruleCode == '10030045' or item.ruleCode == '10190019' or item.ruleCode == '857726' or item.ruleCode == 'sl_id_court_executed' or item.ruleCode == '10030063' or item.ruleCode == 'R10050001' or item.ruleCode=='R10050002' or item.ruleCode=='R10050003' or item.ruleCode=='R10050004' or item.ruleCode=='R10050005' or item.ruleCode=='R10050006' or item.ruleCode=='R10050007' or item.ruleCode=='R10050008' or item.ruleCode=='R10050009' or item.ruleCode=='R10050010'  or item.ruleCode=='10220001'}" >
        ...
        <form id="checkForm" class="layui-form" action="${ctx}/loan/apply/check" method="post">
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <input style="color:red" value="${applyInfo.loanSuccCount }" class="layui-input" maxlength="100" readonly="true"></input>
                </div>
            </div>
...
<script id="tpl_7" type="text/x-handlebars-template">
    <%--客户信息--%>
    <jsp:include page="jdqUserInfo.jsp"></jsp:include>
</script>
<script id="tpl_8" type="text/x-handlebars-template">
    <%--通讯录信息--%>
    <jsp:include page="jdqConnectInfo.jsp"></jsp:include>
</script>
...
<script id="tpl_16" type="text/x-handlebars-template">
    <%--现金白卡报告--%>
    <jsp:include page="jdqData.jsp"></jsp:include>
</script>
...
<script>
    ...
    function applyCheck(form, productId) {
        ...
        // console.log(form.approveTerm.value)
        ...
        if (productId == 'TFL' || productId == 'LYFQ') {
            ...
            if (form.checkResult.value == '1' && form.approveTerm.value == '14' && form.approveAmt.value > '1500') {
                alert("14天产品审批金额不能超过1500");
                return false;
            }
    ...
    function contactConectInfo(applyId, userId) {
        ...
        $.ajax({
            type: "post",
            url: "${ctx}/sys/custUser/getJDQConnectInfo",
            data: param,
            dataType: "json",
            error: function (request) {
                alert("系统繁忙,请稍后再试");
    ...
    function getConnectDetail(applyId) {
        window.open("${ctx}/sys/custUser/getJDQConnectDetail?applyId=" + applyId);
    };
