	...
	<c:if test="${sign == 'check' or sign == 'detail'}">
		...
		<c:if test="${sign == 'check'}">
			...
			<form id="checkForm" class="layui-form"
				action="${ctx}/loan/applyAllot/checkAllot" method="post">
				...
				<c:choose>
					<c:when test="${productId == 'TFL' or productId == 'LYFQ'}">
						<!-- <div class="layui-form-item">
						<label class="layui-form-label">文件:</label>

						<div class="layui-input-inline" style=" width:360px; height: 28px; padding: 5px; margin-bottom: 0;">
					 		<input type="file" id="file1" style=" float:left; width:220px;" />
			        		<input type="button" id="upload" value="上传" style=" float:left;" />
						</div>
					</div> -->
						<div class="layui-form-item">
							<label class="layui-form-label">审批金额</label>
							<div class="layui-input-inline">
								<input placeholder="500-100万" class="layui-input"
									id="approveAmt" name="approveAmt" maxlength="100"></input> <span
									id="bigAmt" style="color: red;"></span>
							</div>
						</div>
						<div class="layui-form-item">
						...
						<div class="layui-form-item">
							...
							<div class="layui-inline">
								<select id="borrowType" name="borrowType">
									...
									<option value="5">质押</option>
									<option value="6">车保宝</option>
									<option value="11">旅游乐</option>
								</select>
							</div>
						</div>
					</c:when>
					<c:when test="${productId == 'CCD' or productId=='ZJD' or productId=='TYD'}">
						<!-- <div class="layui-form-item">
						<label class="layui-form-label">文件:</label>
						...
						<div class="layui-form-item">
							...
							<div class="layui-inline">
								<select id="productId" name="productId" lay-filter="myselect">
									<option value="">请选择产品</option>
									<option value="CCD">诚诚贷（按15天一还）</option>
									<option value="ZJD">租金贷（按月还）</option>
									<option value="TYD">同业贷（按月还）</option>
								</select>
							</div>
						</div>
						<div class="layui-form-item">
							<label class="layui-form-label">审批金额</label>
							<div class="layui-inline">
								<select id="approveAmt" name="approveAmt" lay-filter="myselect2">
									<option value="">请选择审批金额</option>
						...
						<div class="layui-form-item">
							<label class="layui-form-label">审批期数</label>
							<div class="layui-inline">
								<select id="repayTerm" name="repayTerm" lay-filter="myselect1">
									<option value="">请选择审批期数</option>
									<option value="18">18</option>
									<option value="24">24</option>
								</select>
				...
				<div class="layui-form-item mgt-30">
					<div class="layui-input-block">
						<button id="btnCheckSubmit" class="layui-btn" lay-submit=""
							lay-filter="demo1"
...
<script id="tpl_user" type="text/x-handlebars-template">
    <%--客户信息--%>
    <jsp:include page="allotUserInfo.jsp"></jsp:include>
</script>
...
<script>
    $(document).ready(function () {
		...
		}else if(productId=='CCD' || productId=='ZJD' || productId=='TYD'){
			//$("#approveAmt").val(approveAmt);
			//$("#repayTerm").val('${applyInfo.term}');
			//$("#productId").val('${applyInfo.productId}');
    ...
    function applyCheck(form,productId) {
		...
		if(productId=='TFL' || productId=='LYFQ'){
			...
			if (form.checkResult.value == '1' && form.productId.value == '') {
            	alert("请选择产品");
            	return false;
        	}
			if (form.checkResult.value == '1' && form.approveAmt.value == '') {
    ...
    function userInfo(id, applyId, sign) {
        ...
        $.ajax({
            ...
            success: function (data, textStatus) {
                if (data.code == "1") {
                    var myTemplate = Handlebars.compile($("#tpl_user").html());
