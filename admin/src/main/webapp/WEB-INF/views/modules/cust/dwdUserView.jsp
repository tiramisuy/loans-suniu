...
<div class="layui-tab layui-tab-card" style="position:fixed; top:30px;width:100%;z-index: 99999;margin: 0px;" id="floatDiv">
    <ul class="layui-tab-title">
        <c:if test="${sign == 'check' or sign == 'detail' }">
            <li class="layui-this"
                onclick="skipHrefLocation('topAPoint');contactApply('${applyId}','${vo.userInfoVO.id}','${vo.userInfoVO.realName}','${vo.userInfoVO.mobile}')">
                查重信息
            </li>
        </c:if>
        <li onclick="skipHrefLocation('topAPoint');userInfo('${vo.userInfoVO.id}','${applyId}', '${sign}')">客户信息</li>
        <c:if test="${sign == 'check' or sign == 'detail' }">
            <li onclick="skipHrefLocation('topAPoint');contactConectInfo('${applyId}','${vo.userInfoVO.id}')">
                通讯录
            </li>
        </c:if>

        <c:if test="${sign == 'check'}">
            <li onclick="skipHrefLocation('topAPoint');approveView('${applyId}','${vo.userInfoVO.id}','${vo.userInfoVO.realName}','${vo.userInfoVO.mobile}')">
                审批
            </li>
        </c:if>

        <c:if test="${sign == 'user' or sign == 'check' or sign == 'detail'}">
            <li onclick="skipHrefLocation('topAPoint');cardinfoView('${vo.userInfoVO.id}')">银行卡信息</li>
        </c:if>


        <c:if test="${applyInfo.processStatus != 171 && (sign == 'check' or sign == 'detail') }">
            <li onclick="skipHrefLocation('topAPoint');xianJinCardData('${applyId}')">大王贷报告</li>
            <li onclick="skipHrefLocation('topAPoint');anRongData('${applyId}')">安融报告</li>
        </c:if>

        <li>
            <a href="javascript:skipHrefLocation('topAPoint')">${vo.userInfoVO.realName} - ${vo.userInfoVO.mobile}
                - ${channelMap[applyInfo.channelId]}</a>
        </li>
    </ul>
</div>
<div class="layui-tab layui-tab-card" style="top: 70px;position: absolute;width: 100%">
...
<c:if test="${sign == 'check' or sign == 'detail'}">
    ...
    <c:if test="${!empty vo.juqianbao}">
        <table class="layui-table">
            ...
            <c:forEach var="item" items="${vo.juqianbao}" varStatus="xh">
                <c:choose>
                    <%--<c:when test="${item.ruleCode=='857808' or item.ruleCode=='857805' or item.ruleCode=='857798' or item.ruleCode=='857809' or item.ruleCode=='857807' or item.ruleCode=='857802' or item.ruleCode=='857804' or item.ruleCode=='857806' }" >--%>
                    <%--<tr style="color: red;"  <c:if test="${item.sort > 3 or item.sort < 0} "> class="ruleDate" </c:if> >--%>
                    <%--</c:when>--%>
                    <%--<c:when test="${item.ruleCode == '10190003' or item.ruleCode == '10030057' or item.ruleCode == '10030045' or item.ruleCode == '10190019' or item.ruleCode == '857726' or item.ruleCode == 'sl_id_court_executed' or item.ruleCode == '10030063' or item.ruleCode == 'R10050001' or item.ruleCode=='R10050002' or item.ruleCode=='R10050003' or item.ruleCode=='R10050004' or item.ruleCode=='R10050005' or item.ruleCode=='R10050006' or item.ruleCode=='R10050007' or item.ruleCode=='R10050008' or item.ruleCode=='R10050009' or item.ruleCode=='R10050010' }" >--%>
                    <%--<tr style="color: #7400ff;"  <c:if test="${item.sort > 3 or item.sort < 0} "> class="ruleDate" </c:if> >--%>
                    <%--</c:when>--%>
                    <%--<c:when test="${item.ruleCode == '10030106' or item.ruleCode == '10030107'  or item.ruleCode == '10030078' or item.ruleCode == '10030079' or item.ruleCode == '10030082' or item.ruleCode == '10030088' or  item.ruleCode == '10030091' or item.ruleCode == '10030093' or item.ruleCode == '10030094' or item.ruleCode == '10030099' or item.ruleCode == '10030083'--%>
                    <%--or item.ruleCode == '10030110' or item.ruleCode == '10030111' or item.ruleCode == '10030113' or item.ruleCode == '10030115'--%>
                    <%--or item.ruleCode == '10030116' or item.ruleCode == '10030117' or item.ruleCode == '10030124' or item.ruleCode == '10030128' or item.ruleCode == '10030125'--%>
                    <%--or item.ruleCode == '10030123' or item.ruleCode == '10030118' or item.ruleCode == '10030112' or item.ruleCode == '10030108' or item.ruleCode == '10030102' or item.ruleCode == '10030102'}" >--%>
                    <%--<tr style="color: #7400ff;"  <c:if test="${item.sort > 3 or item.sort < 0} "> class="ruleDate" </c:if> >--%>
                    <%--</c:when>--%>
                    <%--<c:when test="${item.ruleCode == '10030063' }" >--%>
                    <%--<tr style="color: #7400ff;"  <c:if test="${item.sort > 3 or item.sort < 0} "> class="ruleDate" </c:if> >--%>
                    <%--</c:when>--%>

                    <c:when test="${item.ruleCode=='857808' or item.ruleCode=='857805' or item.ruleCode=='857798' or item.ruleCode=='857809' or item.ruleCode=='857807' or item.ruleCode=='857802' or item.ruleCode=='857804' or item.ruleCode=='857806' }">
                        <tr style="color: red;" <c:if
                            test="${item.sort > 3 or item.sort < 0} "> class="ruleDate" </c:if> >
                    </c:when>
                    <c:when test="${item.ruleCode == '10030088' or  item.ruleCode == '10030106' or item.ruleCode == '10030107' or item.ruleCode == '10030078' or  item.ruleCode == '10030091' or  item.ruleCode == '10190003' or item.ruleCode == '10030057' or item.ruleCode == '10030045' or item.ruleCode == '10190019' or item.ruleCode == '857726' or item.ruleCode == 'sl_id_court_executed' or item.ruleCode == '10030063' or item.ruleCode == 'R10050001' or item.ruleCode=='R10050002' or item.ruleCode=='R10050003' or item.ruleCode=='R10050004' or item.ruleCode=='R10050005' or item.ruleCode=='R10050006' or item.ruleCode=='R10050007' or item.ruleCode=='R10050008' or item.ruleCode=='R10050009' or item.ruleCode=='R10050010' or item.ruleCode=='10220001' }">
                        <tr style="color: #7400ff;" <c:if
                            test="${item.sort > 3 or item.sort < 0} "> class="ruleDate" </c:if> >
                    </c:when>
                    <c:when test="${ item.ruleCode == '10030079' or item.ruleCode == '10030082' or item.ruleCode == '10030093' or item.ruleCode == '10030094' or item.ruleCode == '10030099' or item.ruleCode == '10030083'
                        or item.ruleCode == '10030110' or item.ruleCode == '10030111' or item.ruleCode == '10030113' or item.ruleCode == '10030115'
                         or item.ruleCode == '10030116' or item.ruleCode == '10030117' or item.ruleCode == '10030124' or item.ruleCode == '10030128' or item.ruleCode == '10030125'
                          or item.ruleCode == '10030123' or item.ruleCode == '10030118' or item.ruleCode == '10030112' or item.ruleCode == '10030108' or item.ruleCode == '10030102' or item.ruleCode == '10030102'}">
                        <tr style="color: red;" <c:if
    ...
    <c:if test="${sign == 'check'}">
        ...
        <form id="checkForm" class="layui-form" action="${ctx}/loan/apply/check" method="post">
            ...
            <div class="layui-form-item">
                <label class="layui-form-label">成功放款次数</label>

                <div class="layui-input-inline">
                    <input style="color:red" class="layui-input" maxlength="100" readonly="true"
                           value="${applyInfo.loanSuccCount }"></input>
                </div>
            </div>
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <select id="approveTerm" name="approveTerm" readonly="true">
                        <option aria-checked="true" value="${applyInfo.approveTerm}">${applyInfo.approveTerm}</option>
                    </select>
                </div>
            </div>
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <select id="servFeeRate" name="servFeeRate">
                        <option value="${applyInfo.servFeeRate}"><fmt:formatNumber value="${applyInfo.servFeeRate}"
                                                                                   pattern="0.00"
            ...
            <div class="layui-form-item layui-form-text">
                ...
                <div class="layui-input-block">
                    <textarea placeholder="100字以内" class="layui-textarea" name="remark" maxlength="100"></textarea>
                </div>
            </div>

            <div class="layui-form-item mgt-30">
                <div class="layui-input-block">
                    <button id="btnCheckSubmit" class="layui-btn" lay-submit="" lay-filter="demo1"
                            onClick="return applyCheck(this.form,'${productId}')">
                        提交
                    </button>
        ...
        <form class="layui-form" action="">
            <div class="layui-form-item mgt-20">
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                            <%--取值最后一条最新的数据展示结果--%>
                        <c:forEach items="${vo.checkLogVOList}" var="checkLog" varStatus="xh">
                            <c:if test="${xh.last}">
                                <%--人工审批通过--%>
                                <c:if test="${checkLog.status == 221}">
                                    <input type="radio" name="checkResult" value="1" title="通过" checked=""
                                           readonly="readonly">
                                    <input type="radio" name="checkResult" value="2" title="拒绝" readonly="readonly">
                                </c:if>
                                <%--人工审批否决--%>
                                <c:if test="${checkLog.status == 222}">
                                    <input type="radio" name="checkResult" value="1" title="通过" readonly="readonly">
                                    <input type="radio" name="checkResult" value="2" title="拒绝" checked=""
                                           readonly="readonly">

                                    <div class="layui-inline">
                                        <label class="layui-form-label" style="padding:0">拒绝原因 ：</label>
                                            ${checkLog.refuse}
                                    </div>
                                </c:if>
                            </c:if>
            ...
            <div class="layui-form-item layui-form-text">
                ...
                <c:forEach items="${vo.checkLogVOList}" var="checkLog" varStatus="xh">
                    <c:if test="${xh.last}">
                        <%--人工审批意见--%>
                        <c:if test="${checkLog.status == 221 || checkLog.status == 222 || checkLog.status == 223}">
                            <div class="layui-input-block">
...
<script id="tpl_7" type="text/x-handlebars-template">
    <%--客户信息--%>
    <jsp:include page="dwdUserInfo.jsp"></jsp:include>
</script>
<script id="tpl_8" type="text/x-handlebars-template">
    <%--通讯录信息--%>
    <jsp:include page="dwdConnectInfo.jsp"></jsp:include>
</script>
...
<script id="tpl_16" type="text/x-handlebars-template">
    <%--现金白卡报告--%>
    <jsp:include page="dwdData.jsp"></jsp:include>
</script>

<script id="tpl_17" type="text/x-handlebars-template">
    <%--安融报告--%>
    <jsp:include page="anRongData.jsp"></jsp:include>
</script>

<script>
    $(document).ready(function () {
        <%--userInfo('${vo.userInfoVO.id}', '${applyId}', '${sign}');--%>
        contactApply('${applyId}', '${vo.userInfoVO.id}', '${vo.userInfoVO.realName}', '${vo.userInfoVO.mobile}');
        ...
        $(window).scroll(function () {
            ...
            // 获得div的高度
            // var h = $("#floatDiv").offset().top;
            // alert(h);
            if (thisH > 60) {
                // 滚动到指定位置
                $("#floatDiv").css({"top":"0px"})
            } else if (thisH < 60) {
                $("#floatDiv").css({"top":"30px"})
            }
        });
    ...
    function applyCheck(form, productId) {
        ...
        if (form.checkResult.value == '3' || form.checkResult.value == '') {
            alert("审批结果请选择通过或拒绝");
            return false;
        }
    ...
    function contactConectInfo(applyId, userId) {
        ...
        $.ajax({
            type: "post",
            url: "${ctx}/sys/custUser/getDWDConnectInfo",
            data: param,
            dataType: "json",
            error: function (request) {
                alert("系统繁忙,请稍后再试");
    ...
    function getConnectDetail(applyId) {
        window.open("${ctx}/sys/custUser/getDWDConnectDetail?applyId=" + applyId);
    };

    function anRongData(applyId) {
        var param = {
            applyId: applyId
        };
        $.ajax({
            type: "post",
            url: "${ctx}/loan/apply/anRongData",
            data: param,
            dataType: "json",
            error: function (request) {
                alert("系统繁忙,请稍后再试");
            ...
            success: function (data, textStatus) {
                if (data.code == "1") {
                    var myTemplate;
                    myTemplate = Handlebars.compile($("#tpl_17").html());
                    var html = myTemplate(data);
                    document.getElementById("tabDiv").innerHTML = html;
                } else {
                    alert(data.msg);
