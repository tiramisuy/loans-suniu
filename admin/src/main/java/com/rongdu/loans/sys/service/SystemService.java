...
import com.rongdu.common.service.ServiceException;
...
import com.rongdu.loans.sys.security.SystemAuthorizingRealm;
import com.rongdu.loans.sys.utils.LogUtils;
...
//		String str = JsonMapper.toJsonString(UserUtils.get(id));
	...
	* @return List<Role>    返回类型  
	...
	public List<User> getUserListByUserIds(List<String> userIds){
		return userDao.findUserByUserIds(userIds);
	}
	...
	/**
	 * 通过部门ID以及所属门店获取用户列表，仅返回用户id和name（树查询用户时用）
	 * 
	 * @param user
	 * @return
	 */
	...
	/**
	 * 通过部门编码以及所属门店获取用户列表，仅返回用户id和name（树查询用户时用）
	 * 
	 * @param user
	 * @return
	 */
	...
	public List<User> findUserByOfficeCode(String code) {
		...
		List<User> list = (List<User>) JedisUtils.getObject("findUserByOfficeCode" + code);
		if (list == null){
			...
			if(officeList != null && officeList.size()>0){
				for(Office officeItem : officeList){
					list.addAll(findUserByOfficeId(officeItem.getId()));
				}
			}
			
			JedisUtils.setObject("findUserByOfficeCode"+ code, list, 5 * 60);
		}
		return list;	
	}
...
//			systemRealm.clearAllCachedAuthorizationInfo();
		...
		if(UserUtils.haveRole(user, "xinshen")){
			CacheUtils.remove("userCache", "role_xinshen");
		}
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public static boolean printKeyLoadMessage(){
		...
		sb.append("\r\n======================================================================\r\n");
		sb.append("\r\n    欢迎使用 "+ Global.getConfig("productName")+"  - "+Global.getConfig("domainName")+"  \r\n");
		sb.append("\r\n======================================================================\r\n");
	...
	public List<User> findUserByOffice(){
		List<User> list = userDao.findUserByOffice();
		return list;
	}
	...
	public List<Map<String, Object>> getUserListByCondition(
		...
		return userDao.getUserListByCondition(condition);
	}

	public List<Map<String, Object>> getCSCompanyListByCondition(
			Map<String, Object> condition) {
		return userDao.getCSCompanyListByCondition(condition);
	}
	 ...
	 * @return
	 */
	public List<User> getUserByRoleAndDept(String role,String... deptCodes){
		return userDao.getUserByRoleAndDept(role,Arrays.asList(deptCodes));
	}
