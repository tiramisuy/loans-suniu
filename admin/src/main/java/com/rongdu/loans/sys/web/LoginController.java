...
import com.rongdu.common.servlet.ValidateCodeServlet;
import com.rongdu.common.utils.CacheUtils;
import com.rongdu.common.utils.CookieUtils;
...
import com.rongdu.loans.sys.security.UsernamePasswordToken;
...
public class LoginController extends BaseController {
	...
	public String ssoLogin(HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		if(StringUtils.isBlank(redirectUrl) || StringUtils.isBlank(username)){
			redirectUrl= adminPath;
		}
		
		// 如果已经登录，则跳转到redirectUrl
		if (principal != null && username.equals(UserUtils.getUser().getLoginName())) {
			return "redirect:" + redirectUrl;
		}

		
		Subject subject = SecurityUtils.getSubject();
		UsernamePasswordToken token = new UsernamePasswordToken(username, "sso123456".toCharArray(),false,StringUtils.getRemoteAddr((HttpServletRequest)request),null,false);		
	...
	public String index(HttpServletRequest request, HttpServletResponse response) {
		Principal principal = UserUtils.getPrincipal();
		boolean toNewIndexPage = !StringUtils.endsWith(request.getRequestURI(), "/a");
		// 登录成功后，验证码计算器清零
