...
import com.rongdu.loans.fileserver.option.UploadDocumentOP;
...
public class UploadDocumentController extends BaseController {
	...
	public ApiResult uploadDocument(HttpServletRequest request, @Valid UploadDocumentOP vo, Errors errors) throws UnsupportedEncodingException {
		if (errors.hasErrors()) {
			throw new BadRequestException(errors);
		}
		String ipAddr = Servlets.getIpAddress(request);
		logger.debug("文档上传：来源于{}",ipAddr);
	...
	private FileInfo createFileInfo(UploadDocumentOP vo) {
		MultipartFile file = vo.getFile();		
