...
import com.rongdu.loans.fileserver.option.UploadImageOP;
...
public class UploadImageController extends BaseController {
	...
	public ApiResult uploadImage(HttpServletRequest request,@Valid UploadImageOP vo,Errors errors) throws UnsupportedEncodingException {
		if (errors.hasErrors()) {
			throw new BadRequestException(errors);
		}
		String ipAddr = Servlets.getIpAddress(request);
		logger.debug("图片上传：来源于{}",ipAddr);
	...
	private FileInfo createFileInfo(UploadImageOP vo) {
		MultipartFile file = vo.getFile();		
