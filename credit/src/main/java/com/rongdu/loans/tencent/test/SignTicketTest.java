...
public class SignTicketTest {
	
	/**
	 * 日志对象
	 */
	protected static Logger logger = LoggerFactory.getLogger(SignTicketTest.class);
	...
	public static void main(String[] args) throws Exception {
		...
		logger.debug("{}-{}-请求地址：{}",TencentConfig.partner_name,TencentConfig.sign_ticket_biz_name,url);
		logger.debug("{}-{}-请求报文：{}",TencentConfig.partner_name,TencentConfig.sign_ticket_biz_name,params);
		long start = System.currentTimeMillis();
		HttpUtils httpUtils = new HttpUtils();
		String responseString = httpUtils.getForJson(url, params);
		long end = System.currentTimeMillis();
		long timeCost = end-start;
		logger.debug("{}-{}-耗时：{}ms",TencentConfig.partner_name,TencentConfig.sign_ticket_biz_name,timeCost);
		logger.debug("{}-{}-应答结果：{}",TencentConfig.partner_name,TencentConfig.sign_ticket_biz_name,responseString);
