...
/**
 * 规则名称：运营商报告呼出前10，与通讯录匹配大于50%
 */
public class AT022Executor extends Executor {
    ...
    private HitRule checkRule(DWDReport report, List<PhoneList> phoneList) {
        ...
        if (report.getCalloutcntlistv() != null) {
            ...
            if (report.getCalloutcntlistv().size() >= 10){
                contactLists = report.getCalloutcntlistv().subList(0, 10);
            } else {
            ...
            if (count <= 5) {
                setHitNum(1);
                remark = String.format("运营商报告呼出前10，与通讯录匹配小于等于0.5：%s", count);
