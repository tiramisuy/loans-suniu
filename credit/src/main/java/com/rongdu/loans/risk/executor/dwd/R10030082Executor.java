 ...
 * @version 2019-05-27
 */
public class R10030082Executor extends Executor {

    @Override
    public void init() {
        super.setRuleId(RuleIds.R10030082);
    }
                ...
                if (StringUtils.isNotBlank(custContactVO.getMobile())) {
                    for (ContactList contact : contactListReport) {
                        ...
                        if (custContactVO.getMobile().equals(contact.getPhoneNum())) {
                            if (contact.getCallCnt() <= 5){
                                setHitNum(1);
                                remark = String.format("近6个月与紧急联系人通话次数小于等于5，紧急联系人号码：%s，近6个月通话次数：%s", custContactVO.getMobile(), contact.getCallCnt());
