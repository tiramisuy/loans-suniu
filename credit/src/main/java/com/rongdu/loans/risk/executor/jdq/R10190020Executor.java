 ...
 * @desc 近6个月关机天数不符合要求
 */
public class R10190020Executor extends Executor {

    @Override
    public void init() {
        super.setRuleId(RuleIds.R10190020);
    }
    ...
    private HitRule check(List<MoxieTelecomReport.ActiveDegreeBean> activeDegreeBeans) {
        ...
        for (int i = 0; i < activeDegreeBeans.size(); i++) {
            if ("".equals(activeDegreeBeans.get(i).app_point)) {
        ...
        hitRule.setRemark(String.format("近6个月关机天数：%d", num));
        return hitRule;
    }
}