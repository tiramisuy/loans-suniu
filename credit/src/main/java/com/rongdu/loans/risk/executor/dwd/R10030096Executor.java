...
/**
 * 近6个月通话时长top10的号码与通讯录名单匹配过低
 *
 * @author fy
 * @version 2019-05-27
 */
public class R10030096Executor extends Executor {

    @Override
    public void init() {
        super.setRuleId(RuleIds.R10030096);
    }
    ...
    private HitRule checkRule(Report report, List<PhoneList> phoneList) {
        ...
        if (report.getContactList() != null) {
            ...
            if (count <= 5) {
                setHitNum(1);
                remark = String.format("近6个月通话时长top10的号码与通讯录名单匹配过低,小于等于5，近6个月通话次数匹配数：%s", count);
