...
/**
 * 规则名称：运营商报告呼入前10，与通讯录匹配大于50%
 */
public class AT019Executor extends Executor {
    ...
    private HitRule checkRule(DWDReport report, List<PhoneList> phoneList) {
        ...
        if (report.getCalloutcntlistv() != null) {
            ...
            List<Calincntlistv> contactLists = null;
            if (report.getCalincntlistv().size() >= 10){
                contactLists = report.getCalincntlistv().subList(0, 10);
            } else {
                contactLists = report.getCalincntlistv();
            }
            // 与通讯录号码比对匹配数
            for (Calincntlistv contact : contactLists) {
            ...
            if (count <= 5) {
                setHitNum(1);
                remark = String.format("运营商报告呼入前10，与通讯录匹配小于等于0.5：%s", count);
