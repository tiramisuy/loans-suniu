...
/**
 * 近6个月呼入总时长的月均值<=0.2小时或>=75小时
 *
 * @author fy
 * @version 2019-05-27
 */
public class R10030091Executor extends Executor {

    @Override
    public void init() {
        super.setRuleId(RuleIds.R10030091);
    }
...
//            if (count6 <= Double.valueOf(0.2) || count6 >= Double.valueOf(75) ) {
            if (count6 <= Double.valueOf(1.3) || count6 >= Double.valueOf(16) ) {
                setHitNum(1);
//                remark = String.format("近6个月呼入总时长的月均值<=0.2小时或>=75小时，近6个月通话次数均值：%s", count6);
                remark = String.format("近6个月呼入总时长的月均值<=1.3小时或>=16小时，近6个月通话次数均值：%s", count6);
