 ...
 * @version 2019-05-27
 */
public class R10030108Executor extends Executor {

    @Override
    public void init() {
        super.setRuleId(RuleIds.R10030108);
    }
    ...
    private HitRule checkRule(Report report) {
        ...
        if (report.getApplicationCheck() != null) {
            ...
            for (ApplicationCheck check : applicationCheck) {
                if ("id_card".equals(check.getAppPoint()) && null != check.getCheckPoints().getFinancial_blacklist()) {
                    ...
                    if (financialBlacklist.arised){
                        setHitNum(1);
                        remark = String.format("运营商金融服务类机构黑名单身份证号检查,用户命中黑名单，列表：%s", financialBlacklist.black_type != null ? financialBlacklist.black_type.toString() : "");
