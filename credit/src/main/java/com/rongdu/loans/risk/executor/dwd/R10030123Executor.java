 ...
 * @version 2019-05-27
 */
public class R10030123Executor extends Executor {

    @Override
    public void init() {
        super.setRuleId(RuleIds.R10030123);
    }
    ...
    private HitRule checkRule(Report report) {
        ...
        if (report.getBehaviorCheck() != null) {
            ...
            for (BehaviorCheck check : behaviorCheck) {
                if ("contact_court".equals(check.getCheckPoint())){
                    if (check.getScore() >= 2) {
                        setHitNum(1);
                        remark = "近6个月与法院号码通话次数过多,多次通话(3次以上)";
