...
public class R10030002Executor extends Executor {
	...
	public void doExecute(AutoApproveContext context) {
		// 加载风险分析数据
		XianJinBaiKaCommonOP vo = getDataInvokeService().getXianJinBaiKaBase(context);
		// 命中的规则
	 ...
	 * @return
	 */
	private HitRule checkMobileRealName(XianJinBaiKaCommonOP op, CustUserVO user) {
		...
		if (!checkUserIDCard(user.getIdNo(), op.getUser_verify().getOperatorVerify().getBasic().getIdcard())) {
			setHitNum(1);
		}
		hitRule.setRemark(String.format("身份证号：%s vs %s", user.getIdNo(),
				op.getUser_verify().getOperatorVerify().getBasic().getIdcard()));
