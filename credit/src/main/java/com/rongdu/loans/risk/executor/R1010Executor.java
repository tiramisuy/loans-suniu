 ...
 * @version 2017-08-14
 */
public class R1010Executor extends Executor {

	@Override
	public void init(){
		super.setRuleId(RuleIds.R1010);
	}

	@Override
	public void doExecute(AutoApproveContext context) {
		CreditDataInvokeService creditDataInvokeService = SpringContextHolder.getBean("creditDataInvokeService");
		ApplyLoandVO vo = creditDataInvokeService.getCredit100ApplyLoand(context);
		//直接命中百融的风险清单
		Map<String,String> thirdpartRiskList = (Map<String,String>)vo;
		//自定义的规则集合
		Map<String,RiskRule> riskConfigMap = creditDataInvokeService.getRiskRuleMap(getRuleId(),context.getModelId());
		Map<String,String> extParams = getDataInvokeService().getBaishiqiExtParams(context);
		//自定义的规则命中百融的风险清单
		List<HitRule> hitRuleList = checkRiskList(thirdpartRiskList,riskConfigMap,extParams);
