...
/**
 * 宜信致诚-阿福共享平台 规则：在阿福共享平台存在贷款当前逾期M2+，拒绝 数据来源于：宜信阿福共享平台
 * 
 * @author liuzhuang
 * @version 2017-11-13
 */
public class R10030040Executor extends Executor {

	@Override
	public void init() {
		super.setRuleId(RuleIds.R10030040);
	}
	...
	/**
	 * 统计申请人在阿福共享平台存在贷款当前逾期M2+
	 * 审批结果为202(批贷已放款)且还款状态为302（逾期），逾期情况为M2、M3、M3+、M6、M6+，
	 * 审批结果字段为approvalStatusCode；还款状态字段为loanStatusCode；逾期情况字段为ocerdueStatus
	 * 
	 * @param loanRecords
	 * @return
	 */
		...
		if (loanRecords != null) {
			for (LoanRecord item : loanRecords) {
				if (approveStatusCode.equals(item.getApprovalStatusCode())
					...
					if ("M2".equals(item.getOverdueStatus())
							|| "M3".equals(item.getOverdueStatus())
							|| "M3+".equals(item.getOverdueStatus())
							|| "M6".equals(item.getOverdueStatus())
							|| "M6+".equals(item.getOverdueStatus())) {
						count = count + 1;
