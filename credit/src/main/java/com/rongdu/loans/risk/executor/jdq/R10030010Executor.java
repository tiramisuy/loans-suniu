...
/**
 * 申请人所在地通话呼出次数比例较低 数据来源于：借点钱报告数据
 *
 * @author sunda
 * @version 2017-08-14
 */
public class R10030010Executor extends Executor {
    ...
    public void doExecute(AutoApproveContext context) {
        ...
        // 命中的规则
        HitRule hitRule = checkCallOutTimePercentage(workArea, list);
        // 决策依据
     ...
     * @return
     */
    private HitRule checkCallOutTimePercentage(String workArea, List<MoxieTelecomReport.ContactRegionBean> list) {
                ...
                if ("contact_region_6m".equals(item)) {
                    for (MoxieTelecomReport.ContactRegionBean.RegionListBean regionListBean : item.region_list) {
                        if (StringUtils.contains(regionListBean.region_loc, workArea)) {
                            callOut = regionListBean.region_dial_cnt_pct;
                        }
                        if (callOut < minPercentage) {
                            msg = String.format("工作地区%s vs：通话地区%s,呼出次数百分比：%s", workArea,
