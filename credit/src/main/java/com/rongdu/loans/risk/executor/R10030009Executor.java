...
/**
 * 申请人所在地通话呼入次数比例较低 数据来源于：白骑士资信云报告数据
 * 
 * @author sunda
 * @version 2017-08-14
 */
public class R10030009Executor extends Executor {
	...
	public void doExecute(AutoApproveContext context) {
		...
		// 手机运营商为电信则不跑此规则
		if (vo != null && vo.getData() != null && vo.getData().getMnoBaseInfo() != null) {
		...
		// 命中的规则
		HitRule hitRule = checkCallInTimePercentage(workArea, dataList);
		// 决策依据
	 ...
	 * @return
	 */
	private HitRule checkCallInTimePercentage(String workArea, List<ReportMnoCca> list) {
		HitRule hitRule = createHitRule(getRiskRule());
		double minPercentage = 0.30;
		...
		if (list != null && workArea != null) {
			...
			for (ReportMnoCca item : list) {
				...
				similarity = IkanalyzerUtils.getSimilarity(workArea, connectCity);
				double percentage = AutoApproveUtils.parsePercentage(item.getCallInCountPercentage());
				msg = String.format("%s vs：%s，相似度：%s,呼入次数百分比：%s", workArea, connectCity, similarity,
						item.getCallInCountPercentage());
				logger.info(msg);
				setHitNum(1);
