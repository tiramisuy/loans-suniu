...
/**
 * 规则名称：运营商报告呼入前20，与通讯录匹配小于等于50%
 */
public class AT020Executor extends Executor {
    ...
    private HitRule checkRule(DWDReport report, List<PhoneList> phoneList) {
        ...
        if (report.getCalincntlistv() != null) {
            ...
            if (report.getCalincntlistv().size() >= 20){
                contactLists = report.getCalincntlistv().subList(0, 20);
            } else {
            ...
            if (count <= 10) {
                setHitNum(1);
                remark = String.format("运营商报告呼入前20，与通讯录匹配小于等于0.5：%s", count);
