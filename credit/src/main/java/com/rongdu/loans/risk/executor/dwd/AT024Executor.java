...
/**
 * 规则名称：运营商报告呼出TOP50短号较少
 */
public class AT024Executor extends Executor {
    ...
    private HitRule checkShortNumber(DWDReport report) {
        ...
        int countIndex = 0;
        if (report != null && report.getCalloutcntlistv() != null) {
            List<Calloutcntlistv> contactList = report.getCalloutcntlistv();
            // 判断号码位数
            for (Calloutcntlistv contact : contactList) {
        ...
        String msg = String.format("运营商报告呼出前50，短号数为：", countIndex);
