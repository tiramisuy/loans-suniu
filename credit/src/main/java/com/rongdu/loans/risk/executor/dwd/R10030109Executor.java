...
/**
 * 身份证组合过其它电话个数大于等于1，小于等于3；
 *
 * @author fy
 * @version 2019-05-27
 */
public class R10030109Executor extends Executor {

    @Override
    public void init() {
        super.setRuleId(RuleIds.R10030109);
    }
    ...
    private HitRule checkRule(Report report) {
        ...
        if (report.getUserInfoCheck() != null && report.getUserInfoCheck().getCheckSearchInfo() != null && report.getUserInfoCheck().getCheckSearchInfo().getIdcardWithOtherPhones() != null) {
            ...
            if (lists != null && lists.size() >= 1 && lists.size() <= 3){
                setHitNum(1);
                remark = String.format("身份证组合过其它电话个数大于等于1，小于等于3，详情列表：%s", lists.toString());
