...
/**
 * 近6个月通话次数top10的号码与通讯录名单匹配小于等于5
 *
 * @author fy
 * @version 2019-05-27
 */
public class R10030087Executor extends Executor {

    @Override
    public void init() {
        super.setRuleId(RuleIds.R10030087);
    }
    ...
    private HitRule checkRule(Report report, List<PhoneList> phoneList) {
        ...
        if (report.getContactList() != null) {
            ...
            if (count <= 5) {
                setHitNum(1);
                remark = String.format("近6个月通话次数top10的号码与通讯录名单匹配小于等于5，近6个月通话次数匹配数：%s", count);
