 ...
 * @version 2019-05-27
 */
public class R10030122Executor extends Executor {

    @Override
    public void init() {
        super.setRuleId(RuleIds.R10030122);
    }
    ...
    private HitRule checkRule(Report report) {
        ...
        if (report.getBehaviorCheck() != null) {
            ...
            for (BehaviorCheck check : behaviorCheck) {
                if ("contact_bank".equals(check.getCheckPoint())){
                    if (check.getScore() >= 2) {
                        setHitNum(1);
                        remark = "近6个月与银行通话“经常被联系,多次通话(3次以上)";
