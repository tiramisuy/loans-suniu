...
/**
 * 宜信致诚-阿福共享平台 规则：在阿福共享平台存在贷款历史逾期M6+，拒绝 数据来源于：宜信阿福共享平台
 * 
 * @author liuzhuang
 * @version 2017-11-13
 */
public class R10030042Executor extends Executor {

	@Override
	public void init() {
		super.setRuleId(RuleIds.R10030042);
	}
	...
	/**
	 * 借款记录里历史逾期M6+次数≥1；历史逾期M6+字段为overdueM6
	 * 
	 * @param loanRecords
	 * @return
	 */
	private HitRule checkOverdueLoanNum(List<LoanRecord> loanRecords) {
		...
		if (loanRecords != null) {
			for (LoanRecord item : loanRecords) {
				int overdueM6 = StringUtils.isNotBlank(item.getOverdueM6()) ? Integer
						.parseInt(item.getOverdueM6()) : 0;
				if (overdueM6 >= 1) {
					count = count + 1;
					String msg = String
							.format("贷款时间：%s,贷款金额：%s,逾期M6+次数: %s",
		...
		String msg = String.format("历史逾期M6+笔数：%s,详情如下：%s", count,
