...
/**
 * Android设备安装了过多可疑的应用程序2 数据来源于：白骑士资信云报告数据
 * 
 * @author liuzhuang
 * @version 2017-11-13
 */
public class R10030037Executor extends Executor {

	@Override
	public void init() {
		super.setRuleId(RuleIds.R10030037);
	}
	...
	private HitRule checkDeviceInfo(Map<String, Object> deviceInfo,
		...
		if (StringUtils.equals("2", source)) {
			...
			if (StringUtils.isNotBlank(appNameString)) {
				...
				if (sensltiveWords.size() >= 20 || forbiddenWords.size() > 0) {
					setHitNum(1);
				}
