...
/**
 * 宜信致诚-阿福共享平台 规则：在阿福共享平台存在2笔贷款当前逾期M1,数据来源于：宜信阿福共享平台
 * 
 * @author sunda
 * @version 2017-08-14
 */
public class R10030038Executor extends Executor {

	@Override
	public void init() {
		super.setRuleId(RuleIds.R10030038);
	}
	 ...
	 * 逾期笔数为2，审批结果为202(批贷已放款)且还款状态为302（逾期），逾期情况为M1,
	 * 
	 * @param loanRecords
	 * @return
	 */
		...
		if (loanRecords != null) {
			for (LoanRecord item : loanRecords) {
				if (approveStatusCode.equals(item.getApprovalStatusCode())
						...
						&& "M1".equals(item.getOverdueStatus())) {
					count = count + 1;
