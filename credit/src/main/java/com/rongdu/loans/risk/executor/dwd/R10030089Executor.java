...
/**
 * 近3个月通话总时长的月均值<=0.2小时或>=75小时
 *
 * @author fy
 * @version 2019-05-27
 */
public class R10030089Executor extends Executor {

    @Override
    public void init() {
        super.setRuleId(RuleIds.R10030089);
    }
    ...
    private HitRule checkRule(Report report) {
        ...
        if (report.getCellBehavior() != null && report.getCellBehavior().get(0).getBehavior() != null) {
            ...
            for (Behavior behaviorss : behaviors) {
                count += behaviorss.getCallInTime();
                count += behaviorss.getCallOutTime();
            }
            Double count3= Double.valueOf(count)/Double.valueOf(3)/Double.valueOf(60);
            if (count3 <= Double.valueOf(0.2) || count3 >= Double.valueOf(75) ) {
                setHitNum(1);
                remark = String.format("近3个月通话总时长的月均值<=0.2小时或>=75小时，近三个月通话次数均值：%s", count3);
