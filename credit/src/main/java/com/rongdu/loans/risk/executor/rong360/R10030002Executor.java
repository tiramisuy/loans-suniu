...
import com.rongdu.loans.loan.option.rong360Model.RongCommonOP;
...
public class R10030002Executor extends Executor {
	...
	public void doExecute(AutoApproveContext context) {
		...
		RongCommonOP op = new RongCommonOP();
		op.setOrderBaseInfo(orderBaseInfo);
		// 命中的规则
		HitRule hitRule = checkMobileRealName(op, context.getUser());
		// 决策依据
	 ...
	 * @return
	 */
	private HitRule checkMobileRealName(RongCommonOP op, CustUserVO user) {
		...
		if (!checkUserIDCard(user.getIdNo(), op.getOrderBaseInfo().getApplydetail().getUserId())) {
			setHitNum(1);
		}
		hitRule.setRemark(String.format("身份证号：%s vs %s", user.getIdNo(),
				op.getOrderBaseInfo().getApplydetail().getUserId()));
	...
	private boolean checkUserIDCard(String IdCard, String xjbkIdCard) {
		if (StringUtils.isNotBlank(xjbkIdCard)) {
			...
			for (String str : nameArr) {
				if (StringUtils.isNotBlank(str) && !IdCard.contains(str)) {
					return false;
				}
			}
		}
