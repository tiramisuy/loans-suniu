 ...
 * @desc 近3个月关机天数不符合要求
 */
public class R10190021Executor extends Executor {

    @Override
    public void init() {
        super.setRuleId(RuleIds.R10190021);
    }
    ...
    private HitRule check(List<MoxieTelecomReport.ActiveDegreeBean> activeDegreeBeans) {
        ...
        for (int i = 0; i < activeDegreeBeans.size(); i++) {
            if ("power_off_day".equals(activeDegreeBeans.get(i).app_point)) {
        ...
        hitRule.setRemark(String.format("近3个月关机天数：%d", num));
        return hitRule;
    }
}