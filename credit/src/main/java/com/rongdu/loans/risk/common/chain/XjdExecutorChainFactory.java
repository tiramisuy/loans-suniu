...
import com.rongdu.loans.risk.executor.R10030060Executor;
import com.rongdu.loans.risk.executor.R10030069Executor;
import com.rongdu.loans.risk.executor.R10030071Executor;
...
public class XjdExecutorChainFactory {
	public static ExecutorChain createExecutorChain(AutoApproveContext context) {
		...
		LoanApplySimpleVO apply = context.getApplyInfo();
		if ("XJBK".equals(apply.getChannelId()) && !"3".equals(apply.getSource())) {
			// 首贷规则
		...
		} else if (userInfo != null && userInfo.getLoanSuccCount() != null && userInfo.getLoanSuccCount() > 0) {
			// 复贷
