 ...
 * @version 2019-05-27
 */
public class R10030081Executor extends Executor {

    @Override
    public void init() {
        super.setRuleId(RuleIds.R10030081);
    }
                ...
                if (StringUtils.isNotBlank(custContactVO.getMobile())) {
                    for (ContactList contact : contactListReport) {
                        ...
                        if (custContactVO.getMobile().equals(contact.getPhoneNum())) {
                            if (contact.getContact3m() <= 2){
                                setHitNum(1);
                                remark = String.format("近3个月与紧急联系人通话次数小于等于2，紧急联系人号码：%s，近3个月通话次数：%s", custContactVO.getMobile(), contact.getContact3m());
