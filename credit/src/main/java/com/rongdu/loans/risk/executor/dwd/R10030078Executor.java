...
/**
 * 申请人近3个月互通过的号码数量小于等于5
 *
 * @author fy
 * @version 2019-05-27
 */
public class R10030078Executor extends Executor {

    @Override
    public void init() {
        super.setRuleId(RuleIds.R10030078);
    }
    ...
    private HitRule checkRule(Report report) {
        ...
        if (report.getContactList() != null) {
            ...
            for (ContactList contact : contactList) {
                if (contact.getContact3m() > 0) {
                    count++;
                }
            }
            if (count <= 7) {
                setHitNum(1);
                remark = String.format("申请人近3个月互通过的号码数量小于等于7，近三个月通话号码数量：%s", count);
