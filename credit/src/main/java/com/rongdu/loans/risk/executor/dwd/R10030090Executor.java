 ...
 * @version 2019-05-27
 */
public class R10030090Executor extends Executor {

    @Override
    public void init() {
        super.setRuleId(RuleIds.R10030090);
    }
    ...
    private HitRule checkRule(Report report) {
        ...
        if (report.getCellBehavior() != null && report.getCellBehavior().get(0).getBehavior() != null) {
            ...
            for (Behavior behaviorss : behaviors) {
                count += behaviorss.getCallOutCnt();
            }
            Double count6= Double.valueOf(count)/Double.valueOf(6);
            if (count6 <= Double.valueOf(15) || count6 >= Double.valueOf(600) ) {
                setHitNum(1);
                remark = String.format("近6个月呼出总次数的月均值过高或过低,月均值<=15或>=600，近6个月通话次数均值：%s", count6);
