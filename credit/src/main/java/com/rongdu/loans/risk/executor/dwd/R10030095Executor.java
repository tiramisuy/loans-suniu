...
/**
 * 手机号组合过其它身份证个数大于等于1，小于等于3
 *
 * @author fy
 * @version 2019-05-27
 */
public class R10030095Executor extends Executor {

    @Override
    public void init() {
        super.setRuleId(RuleIds.R10030095);
    }
    ...
    private HitRule checkRule(Report report) {
        ...
        if (report.getUserInfoCheck() != null && report.getUserInfoCheck().getCheckSearchInfo() != null && report.getUserInfoCheck().getCheckSearchInfo().getPhoneWithOtherIdcards() != null) {
            ...
            if (lists != null && lists.size() >= 1 && lists.size() <= 3) {
                setHitNum(1);
                remark = String.format("手机号组合过其它身份证个数大于等于1，小于等于3，详情列表：%s", lists.toString());
