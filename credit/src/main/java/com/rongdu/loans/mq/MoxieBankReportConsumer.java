 ...
 * @author liuzhuang
 */
@Service("moxieBankReportConsumer")
public class MoxieBankReportConsumer implements ChannelAwareMessageListener {
	...
	@Autowired
	private MoxieBankService moxieBankService;
	@Autowired
	...
	public void onMessage(Message message, Channel channel) throws Exception {
		try {
			...
			logger.debug("魔蝎网银报告通知消息消费者：{}", body);
			// 保存通知记录
			BankReportNotifyVO vo = JsonMapper.getInstance().fromJson(body, BankReportNotifyVO.class);
			moxieNotifyService.saveBankReportNotify(vo);
			...
			if (result) {
				...
				// 获取报告数据
				BankReportOP op = new BankReportOP();
				op.setTaskId(vo.getTask_id());
				BankReportVO report = moxieBankService.getReportData(op);
				// 上传报告
				if (report != null && report.isSuccess()) {
					uploadFile(vo.getUser_id(), report, FileBizCode.MOXIE_BANK_REPORT.getBizCode(), "txt");
				}
			}
		} catch (Exception e) {
			// 被拒绝的消息是否重新入队列
