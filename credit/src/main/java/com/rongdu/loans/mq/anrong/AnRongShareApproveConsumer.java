 ...
 * 安融共享审批结果消费者
 */
@Service("AnRongShareApproveConsumer")
public class AnRongShareApproveConsumer implements ChannelAwareMessageListener {
    @Autowired
    ...
    public void onMessage(Message message, Channel channel) throws Exception {
        try {
            ...
            logger.debug("安融共享审批结果消费者：{}，{}，{}", msg.getSource(), msg.getType(), msg.getBizId());
            ShareVO shareVO = msg.getMessage();
            // 处理数据
            Map<String, String> param = loanApplyService.handleApproveOrder(shareVO, msg.getSource());
            // 发送请求
            if (null != param) {
                ShareResultVO vo = anRongService.sendShareApproveResult(param);
                if (vo != null && vo.getErrors() == null) {
                    applyTripartiteAnrongService.update(shareVO.getLoanId(), param.get("checkResult"));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
