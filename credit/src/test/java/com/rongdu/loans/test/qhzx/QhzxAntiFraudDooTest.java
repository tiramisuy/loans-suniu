...
public class QhzxAntiFraudDooTest {
	
	private static String url = QhzxConfig.domain+"/do/"+QhzxConfig.net_type+"/verify/"+QhzxConfig.antiFraudDoo_biz_code+"/"+QhzxConfig.api_ver+"/"+QhzxConfig.antiFraudDoo_message_code;
	/**
	 * 日志对象
	 */
	protected static Logger logger = LoggerFactory.getLogger(QhzxAntiFraudDooTest.class);
...
//		String ip = "127.0.0.1";
		...
		param.put("idNo", idNo);
		param.put("name", name);
		param.put("reasonNo", "01");
		//0-不核查,1-核查
		param.put("chkBadInfo","0");
		param.put("mobileNo", mobile);
//		param.put("ip", ip);
//		param.put("busiDesc", cardNo);
		...
		logger.debug("{}-{}-请求报文(明文)：{}",QhzxConfig.partner_name,QhzxConfig.antiFraudDoo_biz_name,busiDataString);
		...
		logger.debug("{}-{}-请求地址：{}",QhzxConfig.partner_name,QhzxConfig.antiFraudDoo_biz_name,url);
		logger.debug("{}-{}-请求报文：{}",QhzxConfig.partner_name,QhzxConfig.antiFraudDoo_biz_name,requestString);
		long start = System.currentTimeMillis();
		String responseString = postForJson(url, requestString);
		long end = System.currentTimeMillis();
		long timeCost = end-start;
		logger.debug("{}-{}-耗时：{}ms",QhzxConfig.partner_name,QhzxConfig.antiFraudDoo_biz_name,timeCost);
		logger.debug("{}-{}-应答结果：{}",QhzxConfig.partner_name,QhzxConfig.antiFraudDoo_biz_name,responseString);
		...
		logger.debug("{}-{}-应答结果(明文)：{}",QhzxConfig.partner_name,QhzxConfig.antiFraudDoo_biz_name,busiDataResponseString);
