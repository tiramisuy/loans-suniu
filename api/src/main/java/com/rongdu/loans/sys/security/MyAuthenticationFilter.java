...
public class MyAuthenticationFilter extends AuthorizationFilter {
	...
	@Override
	protected boolean isAccessAllowed(ServletRequest arg0, ServletResponse arg1, Object mappedValue) throws Exception {
		HttpServletRequest request = (HttpServletRequest) arg0;

		if (request.getRequestURI().indexOf("/shop/selectAddress") != -1) {
			return true;
		}
		...
		if (StringUtils.isBlank(userId) || StringUtils.isBlank(token)) {
			...
			// 获取所有请求参数
			Map<String, Object> paramsMap = LoginUtils.getResParameters(request);
			if (null != paramsMap) {
			...
			if (SignUtils.authenticationSign(paramsMap, sign)) {
				...
				throw new IncorrectCredentialsException("签名验证失败,请重新登录");
			}
		}
	}
